...
async fn request_filter(request_state: RequestState, _config: &Config, metadata: &Metadata, stream: StreamProperties, authent: Authentication) -> Flow<String> {
    ...
    //* Log of Client ID  */
    let auth_data = authent.authentication(); 
     match auth_data {
        Some(auth) => {
            if let Some(client_id) = &auth.client_id {
                logger::info!("Client ID: {}", client_id);
            } else {
                logger::warn!("No client_id found in authentication data");
            }
            
            // You can also log other fields
            if let Some(client_name) = &auth.client_name {
        ...
        None => {
            logger::warn!("No authentication data available");
        }
    }

    //* In case of JWT presents */
    ...
    if let RequestData::Continue(request_data_value) = request_data {
      ...
      if !response_status.to_string().starts_with("20"){
        ...
        log_data = json!({
            ...
            "event.duration_upstream":duration_prop,
            "source_port" : v_destinaltion_port,
            "http.response.body.bytes": body_bytes
         });
      }
...
async fn configure(launcher: Launcher, Configuration(bytes): Configuration, metadata: Metadata) -> Result<()> {
    ...
    let filter = on_request(|rs, stream, auth| request_filter(rs, &config, &metadata, stream, auth))
